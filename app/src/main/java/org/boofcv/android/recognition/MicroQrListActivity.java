...
import static org.boofcv.android.recognition.MicroQrDetectActivity.uniqueLock;
 ...
 */
public class MicroQrListActivity extends AppCompatActivity {
    // TODO merge Micro QR and QR list activities together

    // List of all qr codes found in the order they were added to the listview
    List<MicroQrCode> qrcodes = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        synchronized (uniqueLock) {
            for (MicroQrCode qr : MicroQrDetectActivity.unique.values()) {
                qrcodes.add(qr);
        ...
        listView.setOnItemClickListener((adapterView, view, position, l) -> {
            MicroQrCode qr = qrcodes.get(position);
    ...
    public void pressedCopyMessage(View view) {
        ...
        if (message.length() > 0) {
            ...
            ClipData clip = ClipData.newPlainText("microqr", message);
    ...
    public void pressedClearList(View view) {
        synchronized (uniqueLock) {
            MicroQrDetectActivity.unique.clear();
        }
        recreate();
    }
